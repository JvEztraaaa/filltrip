...
import { ScrollView, Text, View, Pressable, useWindowDimensions, Image } from 'react-native';
...
const people = [
  { name: 'Jan Vincent Estrada', role: 'Frontend Developer', imageUrl: require('../assets/images/estrada.png') },
  { name: 'Dian Mendoza', role: 'Backend Developer', imageUrl: require('../assets/images/mendoza.png') },
  { name: 'Gabriel Rola', role: 'Frontend Developer', imageUrl: require('../assets/images/rola.png') },
  { name: 'Gervhee Velez', role: 'Frontend Developer', imageUrl: require('../assets/images/velez.png') },
  { name: 'Mark Gabrielle Dela Cruz', role: 'Backend Developer', imageUrl: require('../assets/images/delaCruz.png') },
  { name: 'Pauline Manaois', role: 'UI/UX Designer', imageUrl: require('../assets/images/manaois.png') },
];
...
export default function AboutPage() {
  ...
  return (
    <ScrollView className="bg-gray-900 min-h-screen" contentContainerStyle={{ paddingBottom: 20 }}>
      {/* Header */}
      ...
      {menuOpen && (
        <View className="bg-gray-800 p-4">
          {['Home', 'About', 'Contact Us'].map((item) => (
            <Pressable key={item} onPress={() => setMenuOpen(false)} className="py-2">
      ...
      <View className="mb-8 items-center px-4">
        ...
        <Text className="mt-2 text-gray-400 text-center text-base">
      ...
      <View className="self-center mt-6 w-full px-5 gap-3">
        {sections.map((s) => (
          <View key={s.id} className="bg-gray-800 border border-gray-700 rounded-lg p-5">
            <Text className={`text-white font-bold mb-1 ${sectionTitleSize}`}>{s.title}</Text>
            <Text className={`text-gray-200 ${sectionDescSize}`}>{s.description}</Text>
          </View>
        ))}
      </View>
      ...
      <View className={`mt-12 mb-12 px-4 ${teamsize}`}>
        <Text className="text-3xl font-semibold text-white mb-4">Meet our Developers</Text>
        <Text className="text-gray-400 mb-6">
          We’re a small team of students who built Filltrip to make fuel tracking and trip planning easier.
        </Text>

        {people.map((person) => (
          <View key={person.name} className="flex-row items-center mb-4 p-3 rounded-lg bg-gray-800">
            <Image
            ...
            />
            <View>
              <Text className="text-white text-[18px] font-semibold">{person.name}</Text>
