 ...
 * refuel_delete.php
 * ------------------------------------------------------------------
 * Deletes a fuel history record by id (POST). Silently succeeds if
 * the row is not owned by user (deleted => false).
 */
...
/* ----------------------------- Delete ------------------------------- */
$del = $pdo->prepare('DELETE FROM fuel_history WHERE id=? AND user_id=?');
$del->execute([$id, $uid]);
