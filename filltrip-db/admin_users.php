 ...
 * admin_users.php
 * ------------------------------------------------------------------
 * Admin CRUD for users (pagination + search by full_name/email/username).
 * Methods: GET (list or single), POST (create), PUT (update), DELETE (remove).
 */
  ...
  switch ($method) {
    case 'GET':
      if (isset($_GET['id'])) {
        ...
        echo json_encode(['success'=>true,'users'=>$users,'currentPage'=>$page,'totalPages'=>$totalPages,'total'=>$total]);
      }
      break;
    ...
    case 'PUT':
      ...
      $stmt->execute([$input['first_name'], $input['last_name'], $fullName, $input['email'], $input['role'], $id]);
