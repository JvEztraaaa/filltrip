 ...
 * - Accepts POST only (JSON or form data)
 * - On success: starts a session and returns user profile (excluding password hash)
 * - On failure: appropriate HTTP status & JSON error.
 */
...
function read_input(): array {
  if (!empty($_POST)) return $_POST; // Form POST fast‑path
...
if ($missing) {
  ...
  echo json_encode(['success'=>false,'error'=>'Invalid credentials','missing'=>$missing]);
  exit;
}
...
unset($row['password_hash']); // Never expose hash
