...
/* lookup */
$sel = $pdo->prepare("SELECT id, full_name AS fullName, username, email, user_icon AS avatarUrl, password_hash, created_at AS createdAt FROM users WHERE email = ? LIMIT 1");
$sel->execute([$email]);
$row = $sel->fetch();
