 ...
 * refuel_list.php
 * ------------------------------------------------------------------
 * Lists fuel history entries for current session user.
 * Returns empty list if not authenticated (non-error).
 */
...
/* ---------------------------- Query Rows --------------------------- */
$sql = "SELECT id, CONCAT(DATE_FORMAT(date, '%Y-%m-%dT%H:%i:%s'), 'Z') AS createdAt, vehicle_name AS vehicleName, odometer_km AS odometerKm, distance_unit AS distanceUnit, liters, fuel_unit AS fuelUnit, price_per_liter AS pricePerLiter, total_cost AS totalCost, fuel_type AS fuelType, station, currency FROM fuel_history WHERE user_id=? ORDER BY date DESC, id DESC";
$st = $pdo->prepare($sql);
$st->execute([$uid]);
$items = $st->fetchAll();
